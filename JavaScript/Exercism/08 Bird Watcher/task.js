...
function birdsInWeek(birdsPerDay, weekNum) {
  
    let chunkOfWeeks = [];
    let birdsPerWeek = 0; 
        
    for (let i = 0, j = 0; i < birdsPerDay.length/7; i ++, j += 7){
          chunkOfWeeks.push(birdsPerDay.slice(j, j + 7));};
    
    for (let k = 0; k < chunkOfWeeks[weekNum-1].length; k++){
         birdsPerWeek = birdsPerWeek + chunkOfWeeks[weekNum-1][k];};
